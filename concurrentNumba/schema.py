"""concurrentNumba/schema.py

CMS Open Payments canonical schema definition.

Responsibilities
----------------
* ``CANONICAL_COLS``        – ordered list of columns in the output dataset.
* ``CMS_COLUMN_MAPPINGS``   – mapping from logical field keys to raw CMS CSV
                              column names, per dataset type.
* ``make_canonical_meta``   – build an empty DataFrame with correct dtypes
                              (used to initialise an empty output when no
                              records pass validation).
* ``get_column_mapping``    – resolve the logical-to-physical column map for a
                              given chunk DataFrame, raising early if required
                              columns are absent.
* ``sanitize_for_parquet``  – coerce all columns to Parquet-compatible dtypes
                              before writing.
"""
...
def get_column_mapping(
    ...
    """Resolve the logical-to-physical column map for a chunk DataFrame.

    Parameters
    ----------
    df:
        The raw chunk read from CSV (column names are the physical CSV headers).
    dataset_type:
    ...
    non-required column is missing from the file).
...
def sanitize_for_parquet(pdf: pd.DataFrame) -> pd.DataFrame:
    """Coerce all DataFrame columns to Parquet-compatible dtypes.

    Column-type rules
    -----------------
    * ``amount_usd``       → float64
    * ``is_product_related``, ``is_valid`` → bool  (NaN filled with False)
    * ``payment_quarter``  → Int8 (nullable)
    * ``program_year``     → Int16 (nullable)
    * Everything else      → ``string[python]`` (None for NaN cells)

    Returns the modified DataFrame (operates in-place on *pdf*).
    """
